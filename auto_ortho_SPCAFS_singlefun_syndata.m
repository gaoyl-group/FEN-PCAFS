...
function [idx_box,W_box]=auto_ortho_SPCAFS_singlefun_syndata(X,Y,mu_candi)
%X:nd
% nKm=20;
[~,fean]=size(X);
...
% lambda_candi=1000:1000:1000000;
lambda_candi = 10.^(-6:1:6);
% mu_candi = 0.5;%靠近1，效果明显                 
s_candi = 1;
p_num = [1];
...
% 
% 
W_box = cell(length(paramCell),1);
idx_box = zeros(fean,length(paramCell));
...
%     [id,~,~,~] = autoSPCAFS(X',paramCell{i1}.lambda,paramCell{i1}.m,paramCell{i1}.mu,paramCell{i1}.p,paramCell{i1}.s);
    [id,~,W,sumw] = auto_ortho_SPCAFS(X,paramCell{i1}.lambda,paramCell{i1}.m,paramCell{i1}.mu,paramCell{i1}.p,paramCell{i1}.s);
    idx_box(:,i1) = sumw;
    W_box{i1}=W;
