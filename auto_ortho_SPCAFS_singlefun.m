function [res_multidim,res_std,rumtime_average,idx_box]=auto_ortho_SPCAFS_singlefun(X,Y,fea_num,nKm)
% function [idx_box]=autoSPCAFS_singlefun231109(X,Y)
%X:nd
% nKm=20;
% [~,fean]=size(X);
...
lambda_candi = 10.^(-4:1:4);

mu_candi = 0.1:0.1:0.9;

s_candi = [0.001,0.01,0.1:0.1:1];
...
parfor i1 = 1:length(paramCell)
    fprintf(['auto_ortho_SPCAFS parameter search %d out of %d...\n'], i1, length(paramCell));
    t0=cputime;%gamma,m,u,p,s
    [id,~,~,~] = auto_ortho_SPCAFS(X,paramCell{i1}.lambda,paramCell{i1}.m,paramCell{i1}.mu,paramCell{i1}.p,paramCell{i1}.s);
...
for q = 1:length(fea_num)
    ...
    parfor u = 1:length(paramCell)
        fprintf(['auto_ortho_SPCAFS cluster evaluation  u%d fea_num%d ...\n'],u,q);
        idx10 = idx_box{u,1};
